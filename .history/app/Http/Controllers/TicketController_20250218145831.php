...
class TicketController extends Controller {
    ...
    public function getAlerts()
    {
        ...
        // Build base query with status conditions and relationships
        ...
        // For r_id 1 and 2, we don't need additional filters as they see all tickets

        $alerts = $query->get();

        // Transform the alerts into the required format
        $formattedAlerts = $alerts->map(function ($ticket) {
            $alertData = [
                't_control_no' => $ticket->t_control_no,
                'created_at' => $ticket->created_at,
                'f_name' => $ticket->requestedBy->u_fname,
                'l_name' => $ticket->requestedBy->u_lname,
            ];

            // Set alert template based on status
            switch ($ticket->status) {
                case 0:
                    $alertData['template'] = [
                        'bg_class' => 'bg-primary',
                        'icon' => 'fas fa-file-alt',
                        'message' => "A new ticket has been created by {$alertData['f_name']} {$alertData['l_name']}."
                    ];
                    break;
                case 1:
                    $alertData['template'] = [
                        'bg_class' => 'bg-success',
                        'icon' => 'fas fa-donate',
                        'message' => "Your ticket has been approved by {$alertData['f_name']} {$alertData['l_name']}."
                    ];
                    break;
                case 2:
                    $alertData['template'] = [
                        ...
                        'message' => "The ticket {$alertData['t_control_no']} is pending for evaluation."
                    ];
                    break;
            }
