    ...
    public function getAlerts()
{
    ...
    // Count all tickets with status 0, 1, 2 for the current user if r_id = 3
    $query = Ticket::whereIn('status', [0, 1, 2]);
    
    // Modify query based on user role
    if ($user->r_id == 3) {
        $query->where('req_by', $user->u_id); // Limit to only tickets created by the user
    }

    // Remove assigned tickets for Super Admin and Admin
    if (in_array($user->r_id, [1, 2])) {
        $query->whereNull('assigned_to'); // Remove tickets that have been assigned
    }

    $totalCount = $query->count();

    // For Approval Tickets (status 0)
    $forApprovalTickets = Ticket::with(['requester'])
        ->where('status', 0);

    if ($user->r_id == 3) {
        $forApprovalTickets->where('req_by', $user->u_id); // Filter based on user's tickets
    } elseif (in_array($user->r_id, [1, 2])) {
        $forApprovalTickets->whereNull('assigned_to'); // For Admin or Super Admin, remove assigned tickets
    }

    $forApprovalTickets = $forApprovalTickets->get();

    // In Progress Tickets (status 1)
    $inProgressTickets = Ticket::with(['assignedStaff'])
        ->where('status', 1);
    if ($user->r_id == 3) {
        $inProgressTickets->where('req_by', $user->u_id); // Filter based on user's tickets
    } else {
        $inProgressTickets->where('assigned_to', $user->u_id); // Only show assigned tickets to the user
    }
    $inProgressTickets = $inProgressTickets->get();

    // For Evaluation Tickets (status 2)
    $forEvaluationTickets = Ticket::where('status', 2);
    if ($user->r_id == 3) {
        $forEvaluationTickets->where('req_by', $user->u_id); // Filter based on user's tickets
    } else {
        $forEvaluationTickets->where('assigned_to', $user->u_id); // Only show assigned tickets to the user
    }
    $forEvaluationTickets = $forEvaluationTickets->get();

    // Process data for response
    $alerts = [
        'total_count' => $totalCount,
        'for_approval' => $forApprovalTickets->map(function ($ticket) {
            return [
                't_control_no' => $ticket->t_control_no,
                'f_name' => $ticket->requester->u_fname,
                'l_name' => $ticket->requester->u_lname,
                'url' => route('ticket_info', $ticket->t_id)
            ];
        }),
        'in_progress' => $inProgressTickets->map(function ($ticket) {
            return [
                't_control_no' => $ticket->t_control_no,
                'f_name' => $ticket->assignedStaff->u_fname ?? 'Admin',
                'l_name' => $ticket->assignedStaff->u_lname ?? 'User',
                'url' => route('ticket_info', $ticket->t_id)
            ];
        }),
        'for_evaluation' => $forEvaluationTickets->map(function ($ticket) {
            return [
                't_control_no' => $ticket->t_control_no,
                'url' => route('ticket_info', $ticket->t_id)
            ];
