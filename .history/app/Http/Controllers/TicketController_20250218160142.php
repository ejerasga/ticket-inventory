...
/*************  ✨ Codeium Command 🌟  *************/
    ...
    {
        // If requester, only get their own tickets
        if (Auth::user()->r_id == 3) {
            $tickets = Ticket::where('req_by', Auth::user()->u_id);
        } else {
            $tickets = Ticket::whereIn('status', [0, 1, 2]);
        }

        // Count all tickets with status 0, 1, 2
        $totalCount = $tickets->count();
        $totalCount = Ticket::whereIn('status', [0, 1, 2])->count();

        // For Approval Tickets (status 0)
        $forApprovalTickets = $tickets->where('status', 0)
            ->with(['requester'])
        ...
        $inProgressTickets = $tickets->where('status', 1)
            ->with(['assignedStaff'])
        ...
        $forEvaluationTickets = $tickets->where('status', 2)
            ->get();
        $forEvaluationTickets = Ticket::where('status', 2)->get();
...
/******  7edd2ec9-eb1e-4683-9a69-bc19eb148723  *******/
