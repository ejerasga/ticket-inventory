...
class TicketController extends Controller {
    ...
    public function getAlerts()
    {
        ...
        // For regular users (r_id = 3), show only their requested tickets
        if ($user->r_id == 3) {
            $query->where('req_by', $user->u_id);
        }
        // For admin users (r_id = 1 or 2), show:
        // 1. Unassigned tickets (assigned_to is null)
        // 2. Tickets assigned specifically to them
        else {
            $query->where(function($q) use ($user) {
                $q->whereNull('assigned_to')
                ->orWhere('assigned_to', $user->u_id);
            });
        }
