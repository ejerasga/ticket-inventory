...
class TicketController extends Controller {
    ...
    public function getIndexCounts() {
        ...
        if ($user->r_id == 1 || $user->r_id == 2) {
            ...
            $countPendingRequests = TicketModel::where('u_id', $user->u_id)->where('initial_status', 0)->count();
            $countOngoingRequests = TicketModel::where('u_id', $user->u_id)->where(function ($query) {
        ...
        $pendingRequestsForApproval = TicketModel::where('initial_status', 0)
            ->orderBy('req_id', 'asc')
            ->limit(5)
            ->get();
    
        // Fetch the first five pending requests for the "In Progress" card
        $requestsInProgress = TicketModel::where('initial_status', '>', 0)
