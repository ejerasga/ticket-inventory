    ...
    public function getAlerts()
{
    ...
    if (in_array($user->r_id, [1, 2])) {
        // Super Admin & Admin: See all tickets with specified statuses
        $alerts = Ticket::whereIn('status', [0, 1, 2])->with(['created_by', 'approved_by'])->get();
    } else {
        // Regular user (r_id = 3): Only see their own tickets
        $alerts = Ticket::where('req_by', $user->u_id)
                        ->whereIn('status', [0, 1, 2])
                        ->with(['created_by', 'approved_by'])
                        ->get();
    }

    return view('layout.header', compact('alerts'));
}
