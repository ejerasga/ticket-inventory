...
/*************  ✨ Codeium Command 🌟  *************/
    ...
    {
        $count = Ticket::where('assigned_to', Auth::user()->u_id)
        $ticket = Ticket::where('assigned_to', Auth::user()->u_id)
            ->where('status', 1)
            ->count();
            ->get();

        return response()->json(['alerts' => $count]);
        return response()->json(['alerts' => count($ticket)]);
    }
/******  eafcfaa6-6e0c-431a-af1d-74249cec7278  *******/
