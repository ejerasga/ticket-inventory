...
<body>
    ...
    fetch('{{ route("alerts") }}')
        ...
        .then(data => {
            ...
            badge.textContent = data.total_count + (data.total_count > 0 ? '+' : '');

            const dropdown = document.querySelector('.dropdown-list');
            const showAllLink = dropdown.querySelector('.dropdown-item.text-center');

            // Clear existing alerts
            dropdown.querySelectorAll('.alert-item').forEach(el => el.remove());

            // Function to create an alert element
            function createAlert(ticket, bgClass, iconClass, message) {
                const alertItem = document.createElement('a');
                alertItem.className = 'dropdown-item d-flex align-items-center alert-item';
                alertItem.href = ticket.url;
            ...
            data.for_approval.forEach(ticket => {
                ...
                const alert = createAlert(ticket, 'bg-primary', 'fas fa-file-alt', message);
                dropdown.insertBefore(alert, showAllLink);
            });

            // Add In Progress alerts
            data.in_progress.forEach(ticket => {
                const message = `Your ticket has been approved by ${ticket.f_name} ${ticket.l_name}.`;
                const alert = createAlert(ticket, 'bg-success', 'fas fa-donate', message);
                dropdown.insertBefore(alert, showAllLink);
            });

            // Add For Evaluation alerts
            data.for_evaluation.forEach(ticket => {
                const message = `The ticket ${ticket.t_control_no} is pending for evaluation.`;
                const alert = createAlert(ticket, 'bg-warning', 'fas fa-exclamation-triangle', message);
                dropdown.insertBefore(alert, showAllLink);
            });
            ...
            <footer class="sticky-footer bg-white">
                <div class="container my-auto">
                    <div class="copyright text-center my-auto">
                        <span>Copyright &copy; Fuji-Haya Electric</span>
                    </div>
                </div>
            </footer>
            <!-- End of Footer -->
