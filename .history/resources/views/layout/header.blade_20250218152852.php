                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item dropdown no-arrow mx-1">
                            <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button"
                                ...
                                <!-- Counter - Alerts -->
                                <span class="badge badge-danger badge-counter">{{ is_array($alerts) || is_countable($alerts) ? count($alerts) : 0 }}</span>
                            </a>
                            <!-- Dropdown - Alerts -->
                            <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
                                ...
                                @foreach ($alerts as $alert)
                                    @if (in_array($alert->status, [0, 1, 2]))
                                        <a class="dropdown-item d-flex align-items-center" href="{{ route('ticket_info', $alert->t_id) }}">
                                            <div class="mr-3">
                                                <div class="icon-circle 
                                                    {{ $alert->status == 0 ? 'bg-primary' : ($alert->status == 1 ? 'bg-success' : 'bg-warning') }}">
                                                    <i class="fas 
                                                        {{ $alert->status == 0 ? 'fa-file-alt' : ($alert->status == 1 ? 'fa-donate' : 'fa-exclamation-triangle') }} text-white"></i>
                                                </div>
                                            </div>
                                            <div>
                                                <div class="small text-gray-500">{{ $alert->t_control_no }}</div>
                                                <span>
                                                    @if ($alert->status == 0)
                                                        A new ticket has been created by {{ $alert->created_by->f_name }} {{ $alert->created_by->l_name }}.
                                                    @elseif ($alert->status == 1)
                                                        Your ticket has been approved by {{ $alert->approved_by->f_name }} {{ $alert->approved_by->l_name }}.
                                                    @elseif ($alert->status == 2)
                                                        The ticket {{ $alert->t_control_no }} is pending for evaluation.
                                                    @endif
                                                </span>
                                            </div>
                                        </a>
